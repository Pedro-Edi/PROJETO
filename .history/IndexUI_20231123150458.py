    ...
    def sidebar():
        ...
        if  "cliente_id" not in st.session_state:
            ...
            if st.session_state['cliente_nome']=="admin":
                op = st.sidebar.selectbox("Menu", ["Manter Clientes", "Manter Servicos","Manter Agenda","Abrir Agenda do Dia","Serviço Reajuste","Confirmar Agendamento"])
                ...
                op = st.sidebar.selectbox("Menu", ["Abrir Conta","Agenda de Hoje","Agendar horário","Visualizar agendaementos"])
                st.sidebar.write("Bem-vindo(a), " + st.session_state["cliente_nome"])
                if op == "Abrir Conta": AbrirContaUI.main()
                if op == "Agenda de Hoje": AgendaHojeUI.main()
                if op == "Agendar horário": AgendarHorarioUI.main()
                if op == "Visualizar agendaementos": VisualizarAgendamentoUI.main()
                if st.sidebar.button('logout'):
                     del st.session_state["cliente_id"]
                     st.rerun()
