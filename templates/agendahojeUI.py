...
class AgendaHojeUI:
    ...
    def disponiveis():
        ...
        if len(agendas)==0: st.write('Nenhuma agenda para hoje')
        else:
            dic=[]
    ...
    def pendentes():
        ...
        hoje =datetime.today()
        agendas = View.agenda_hoje()
        if len(agendas)==0: st.write('Nenhuma agenda pendente para hoje')
        else:
            dic=[]
            for agenda in agendas:
    ...
    def realizados():
        hoje =datetime.today()
        agendas = View.agenda_hoje()
        if len(agendas)==0: st.write('Nenhuma agenda realizada hoje')
        else:
            dic=[]
            for agenda in agendas:
                if agenda.get_confirmado()==True and agenda.get_data()<hoje and agenda.get_Cliente() ==  st.session_state['cliente_id'] and agenda.get_Servico() != 0:
